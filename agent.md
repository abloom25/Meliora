...
- **部署平台规范**:
  ...
  - **环境变量(2 必填 + 1 条件必填)**:`GH_TOKEN`(GitHub PAT,Contents Read and write)、`CONFIG_ENCRYPTION_KEY`(配置加密、Cookie 签名、构建期公开配置解密的主密钥,至少 32 位,生产模式校验强度——拒绝全相同字符 / 纯顺序字符等弱模式)为必填;`GH_REPO`(`owner/repo` 格式)为条件必填,Vercel 开启 System Environment Variables 后可从 `VERCEL_GIT_REPO_OWNER` / `VERCEL_GIT_REPO_SLUG` 自动推导,其他平台必须填写。环境就绪提示会校验 `GH_TOKEN` 是否可用、`GH_REPO` 格式与 `CONFIG_ENCRYPTION_KEY` 强度;`GH_TOKEN` 缺失或占位会进入 env-not-ready。`GH_BRANCH` 可选(默认 `main`),`GITHUB_PROXY` 可选且必须是公网 HTTPS URL,`ADMIN_DISABLED` 可选(设为 `true`/`1`/`yes`/`on`,大小写不敏感;禁用管理后台时除状态探针(`GET /api/setup-status`、`GET /api/status-page`,返回 HTML 状态页)外,所有 `/api/*` 返回 403,`/admin` 显示"已禁用")。
    ...
    - 开发模式:`DEVELOPMENT=true` 时进入内存模式(配置 / 密码不持久化,加密与签名走明文降级)。**托管生产平台下该变量不生效**:检测到 `VERCEL_ENV=production`、Netlify `CONTEXT=production` 或 Cloudflare Pages 运行时(`CF_PAGES`)时 `isDevelopmentMode` 恒为 false,防止开发变量误带到公网部署导致鉴权/加密降级。
  - **安全头一致性**:所有平台的 headers 配置必须保持一致(CSP、X-Content-Type-Options、X-Frame-Options、Referrer-Policy、Permissions-Policy)。API(Function)响应的安全头由 `server/core/router.ts` 出口统一注入(`X-Content-Type-Options: nosniff` + `Cache-Control: no-store`),因为 Cloudflare Pages 与 Netlify 的平台级 headers 不作用于 Function 响应。
