...
上传站点图标或本地音乐资源时,后台先创建不挂载到分支的暂存 Git Blob；只有点击保存后,加密的 `public/config.json`、全部暂存文件和配置中不再引用的旧资源才会通过同一个 Git Tree / Commit 原子写入目标分支。撤销或保存冲突不会提前覆盖、删除线上文件。提交成功后会触发部署平台自动重新构建；播放器公开配置在构建期写入前端 bundle,首屏不再请求管理 API,远程歌单仍按 `apiEndpoint` 与歌单 ID 实时请求 Meting API。
...
>
> - **密码**:通过 `/setup` 页面设置,经 PBKDF2(600k 迭代)哈希后存储在仓库 `public/admin.json`,后续可在后台修改
