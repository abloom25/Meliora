...
- **进度条歌词预览支持高级歌词**:悬停进度条的气泡改用与歌词面板同一套场景解析,
  对唱双声部同时显示(第二声部靠右)、背景和声附在所属句子下方(更小更淡)、
  罗马音与译文一并显示,并按悬停时刻渲染逐字进度快照;落在间奏里时整体压暗

### Changed

- **歌词动画引擎重写**:版面改为由 JS 全权掌管,每一行绝对定位、各自一根弹簧,容器本身不再
  滚动。换行时沿运动方向逐行错峰起步(Apple Music 那种"被拽过去"的牵引感),错峰间隔按
  本次位移收窄,相邻行不会叠在一起;打断只改目标不改速度,连续换行与 seek 都不再顿挫。
  拖进度条或点击很远的一行时不再拖着整首歌飞过去:行程封顶在半屏出头,新位置的行从视口
  边缘之外级联进场。滚轮 / 触摸 / 键盘浏览由面板自己接管,触摸带惯性,空闲 3.2s 后回到
  跟随;Tab 落到视口外的行会自动带进来
- **背景和声的出现与收回**:和声行只在所属主句唱到时才从主句下沿探出来,主句唱完随之收回。
  占位高度与视觉(自下而上的裁切、淡出、模糊、轻微缩小)由引擎里同一根临界阻尼弹簧驱动,
  下面的行 1:1 跟随实际占位,收回过程中永远压不到还没消失的和声,也不会顶进主句
- **歌词弹簧设置**:新增「歌词弹簧」滑块(0.5–2),按刚度理解,越大越紧绷利落

### Fixed

- 面板自身重渲染(切浏览态、开关动画)后行节点引用会丢失,导致对齐偶发失效;改为直接
  从 DOM 读取行节点
- 几百行的长歌词(带译文的 700+ 行)每次换行卡顿数百毫秒:换行时给视口外的行也重写了
  位置,每个元素的样式重算乘起来就是整帧的代价;现在视口外的行不写位置,进场时再补
- 切歌时旧歌词全部堆到顶上叠成一团再淡出:切歌不再清旧行的位置,旧行原地淡出
- 每秒换好几行的滚动字幕式歌词(Credits EX)活跃行一直追不到锚点:错峰只按视口内的行
  排序、等待中的行不再被反复重排满延迟,整道波与弹簧的收敛时间都按到下一行的剩余时间压缩
...
- **闪光延迟微调**:设备报的输出延迟不一定准(蓝牙编解码延迟常常不在里面),新增
  「闪光延迟」滑块,−100 到 +300ms 叠加在自动补偿之上,闪光比声音早就调大、晚就调小
- **节奏闪光开关**:与「动态封面背景」分开,可以只留封面背景不要闪光,也可以关掉封面
  背景只留随节奏呼吸的主题色光晕
- **闪烁频率设置**:背景节奏闪光新增「闪烁频率」滑块,四个吸附档位(每 2 拍 / 每拍 /
  每半拍 / 每 ¼ 拍),控制闪光的最小间距;原「节奏亮度」滑块继续控制幅度
- **逐字歌词**:接入 AMLL TTML DB(社区维护的 Apple Music 规格逐词歌词库),按平台歌曲 ID
  直连三个带 CORS 的镜像,命中即使用真实字级时间轴;同时支持增强型 LRC 的 `<mm:ss.xx>`
  内联音节、网易云 YRC 与 QQ QRC。歌词格式按内容特征识别而非扩展名,后台上传的 `.lrc`
  里若是 QRC 或 TTML 一样能解析
- **逐字渲染**:英文按词、中文按字推进,唱过的部分随进度渐强发光并保持上浮(不回弹);
  前沿柔化宽度随进度收敛,未唱与已唱的词都是实色。同时支持 TTML 的译文、罗马音、
  对唱声部(左右分栏)与背景和声(附在所属句子下方,与主句一同高亮)
- **歌词小窗逐字同步**:小窗跑自己的帧循环并维护独立的外推时钟——主窗口切到后台时
  `requestAnimationFrame` 会被节流到约 1Hz,共用主窗口的循环会让扫光停摆

### Changed

- **背景随音乐高亮重做**:改为一条连续的节奏包络,不再做击点判定。实时谐波 / 打击分离
  (Fitzgerald 2010 的中值滤波 HPSS,按 Stark 等 2013 做成因果实时版,`utils/hpss.ts`)
  取出打击成分,只看它在 30–150Hz 的线性功率上升量,自适应定标后做快起慢落的包络
  (`utils/beat-envelope.ts`);γ 压缩让每一拍亮度接近,释放时长跟随拍长。速度估计改为
  去均值自相关加谐波家族先验(`utils/tempo-ruler.ts`),修掉旧实现无论什么歌都锁在
  121 BPM 附近的问题,但它只用于释放时长与闪烁频率闸门,不决定何时闪。用 9 首真歌
  (EDM、合成器流行、放克、说唱、软摇滚、民谣、钢琴独奏、抒情、流行)离线验证:有鼓的歌
  拍点覆盖 80–94%,拍外误闪每秒 0.2 次左右,亮度占空 10–20%。画面按 `baseLatency +
outputLatency` 减去视觉链路固有延迟延后写入,蓝牙耳机等高输出延迟设备上闪光不再先于声音
- **背景高亮不再每帧重栅格化**:模糊封面层的 filter / opacity / transform 全部静态,节奏响应
  收敛到独立高亮层的 opacity 与 transform(合成器属性),每帧写入的属性不再挂 transition,
  消除了画面抖动与整屏重绘
- **歌词牵引滚动改为弹簧引擎**:位移由可打断的弹簧驱动,打断时保留当前速度,不再
  cancel 后从 0 速度重建整套关键帧(副歌连续换行时的顿挫来源)。逐行起步的牵引波保留,
  但按距下一句的剩余时间压缩,避免快段落里相邻行拉开距离后交叠
- **高亮不再提前于音频**:滚动保留 0.32s 提前量以便弹簧停下时正好开唱,行高亮与逐字
  扫光则严格对齐音频时间
