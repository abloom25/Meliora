import type { EqualizerSettings, EqPresetId } from '../types'
...
/**
 * 频段的滤波器类型。取值刻意与 Web Audio 的 BiquadFilterType 对齐,但**不引用**它 ——
 * 核心层不该依赖浏览器类型;由平台层在建节点时直接用这个字符串即可
 */
export type EqBandFilterType = 'lowshelf' | 'peaking' | 'highshelf'

export function bandFilterType(index: number): EqBandFilterType {
